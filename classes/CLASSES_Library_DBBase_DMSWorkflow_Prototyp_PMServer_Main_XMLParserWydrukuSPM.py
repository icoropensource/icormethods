...
class ICORSOKPrzesylkiParser(XMLUtil.ICORBaseXMLParser):
   def ParseFile(self,aprocessor,afpath,ret=None,acompressed=0,adomyslnystatusprzesylki='K',adomyslnystatuslisty='K',adomyslnytrybdoreczenia='Ordynacja Podatkowa',anolist=0,anadawca='',awydzialmerytoryczny='',alistafilterfunc=None,aprzesylkafilterfunc=None):
      ...
      aprocessor.aCurrentXMLParser=self
...
#    wersja='1.0.1'>
   ...
   def start_PRZESYLKI(self,attrs):
      if attrs.has_key('BrakList'):
         self.NoList=int(attrs['BrakList'])
      if attrs.has_key('DomyslnyStatusPrzesylki'):
         self.DomyslnyStatusPrzesylki=attrs['DomyslnyStatusPrzesylki']
      if attrs.has_key('DomyslnyTrybDoreczenia'):
         self.DomyslnyTrybDoreczenia=attrs['DomyslnyTrybDoreczenia']
      if attrs.has_key('Nadawca'):
         self.Nadawca=attrs['Nadawca']
      if attrs.has_key('WydzialMerytoryczny'):
         self.WydzialMerytoryczny=attrs['WydzialMerytoryczny']
...
#            attrs['AdresatAdres']=ADOLibInit.GetRSValueAsStr(rs,'KodPocztowy')+' '+attrs.get('AdresatAdresMiejscowosc','')+' '+ADOLibInit.GetRSValueAsStr(rs,'AdresEwidencyjny')+' '+attrs.get('AdresatAdresNrLokalu','')
      ...
      if not self.PrzesylkaFilterFunc is None:
         attrs['DomyslnyStatusPrzesylki']=self.DomyslnyStatusPrzesylki #$$SK 20091102
      ...
      rs=self.PaymentsProcessor.GetRS(asql)
      print 'PRZESYLKA:',attrs.get('IDPrzesylki',''),attrs.get('IDTransakcji','')
      if rs.State!=self.adoconst.adStateClosed:
         if rs.EOF or rs.BOF:
            rs.AddNew()
            print '  NOWA' 
         ...
         anadawca=attrs.get('Nadawca',self.Nadawca)
         if not anadawca:
            anadawca=self.Nadawca
         rs.Fields.Item('Nadawca').Value=anadawca
         rs.Fields.Item('WydzialMerytoryczny').Value=attrs.get('WydzialMerytoryczny',self.WydzialMerytoryczny)
         adatanadania=attrs.get('DataNadania','1900-01-01')
         if not adatanadania:
            adatanadania='1900-01-01'
         rs.Fields.Item('DataNadania').Value=adatanadania
         aDataRejestracjiUSortownika=attrs.get('DataRejestracjiUSortownika','1900-01-01')
         if not aDataRejestracjiUSortownika:
            aDataRejestracjiUSortownika='1900-01-01'
         rs.Fields.Item('DataRejestracjiUSortownika').Value=aDataRejestracjiUSortownika
         ...
         rs.Fields.Item('CzyBezKontroliDatyPrzesylki').Value=attrs.get('CzyBezKontroliDatyPrzesylki','0')
