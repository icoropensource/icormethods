...
"""%(amenu.oid,amenu.Reports.OID,random.randint(1,10000000)))
   ...
   while aobj:
      if CheckRecursiveAccessLevelForUser(aobj,'AccessLevelView',amenu.uid):
         file.write('<option value="%d">%s</option>'%(aobj.OID,aobj.Nazwa))
      aobj.Next('Nazwa')
   file.write("""
</SELECT></td></tr>
</table>
...
def DoGenerateSubmit(amenu,areport,file):
   ...
   if not aobj:
      ...
      return
   file.write('<h1><font color="green">Projekt "%s" jest w trakcie generowania</font></h1><br><button onclick="history.back();">Powrót</button>'%aobj.Nazwa)
   astate=ICORSync.ICORState(aname='STRUKTURY',avalue='RUN')
   aobj.Class.Main('0!'+str(astate.OID),-1,aobj.Nazwa)
   file.write("""
<script language="javascript">
getParentFrame('NAVBAR').registerStateBadOK(%d);
</script>
"""%astate.OID)
#   file.write('&nbsp;&nbsp;<button onclick="javascript:window.location=\'icormain.asp?jobtype=objectedit&CID=%d&OID=%d&menuoid=%d\';" tabIndex=%d>Przejdź do projektu</button>'%(aobj.CID,aobj.OID,amenu.oid,2))
