...
def slimmer(code, syntax=XHTML, hardcore=False):
    ...
    if syntax == XHTML:
        ...
        return js_slimmer(code, hardcore)
...
html_comments_oneline = re.compile(r'<!--(?! *\#).*?-->', f_IMD) 
...
vbany_tag = re.compile(r"<\%.*?\%>", f_IMD)

def _html_slimmer(html, xml=0):
    # 0. preserve VBScript
    dtags={}
    for i,tag in enumerate(vbany_tag.findall(html)):
        if not dtags.has_key(tag):
            srepl='<vbscript_%d/>'%i
            dtags[tag]=srepl
            html=html.replace(tag,srepl)
    ...
    for as1,ac1,as2 in html_inline_js.findall(html):
        html = html.replace(as1+ac1+as2, as1+js_slimmer(ac1)+as2)
    ...
    for tag in uniqify(any_tag.findall(html)):
        ...
        if len(tag)<2:
            continue
        if not ((tag[1]>='a' and tag[1]<='z') or (tag[1]>='A' and tag[1]<='Z')):
            continue
        if tag.find('</')>-1:
            continue
        original = tag
    ...
    # restore VBScript
    for tag,srepl in dtags.items():
        html=html.replace(srepl,tag)
...
sloppy_ifs = re.compile(r'(\})\s*(if|else if|else)\s*({|\()',re.MULTILINE|re.DOTALL)
...
whitespaces_before_closing = re.compile(r'\s+([\)\(\=\;\{\}\[\]\<\>\/\\\!\%\*\-\+\|\?\:\,\&\^\.])', re.MULTILINE|re.I)
whitespaces_after_closing = re.compile(r'([\(\=\;\{\}\[\]\<\>\/\\\!\%\*\-\+\|\?\:\,\&\^\.])\s+', re.MULTILINE|re.I)
separators_after_closing = re.compile(r'([\)])([^\;\}\{\.\)\(\[\]\^\&\*\-\=\+\,\?\/\|\\]\:\<\>\!)', re.MULTILINE)
unnecessarylfs_after_statements = re.compile(r'(\W)(else)\n(\w)', re.MULTILINE)
unnecessary_spaces = re.compile(r'(\W) +(\w)|(\w) +(\W)', re.MULTILINE)
...
#        js = js_function_slimmer(js)
    ...
    # x. unnecessary spaces
    js = whitespaces_before_closing.sub(r'\1', js)
    js = whitespaces_after_closing.sub(r'\1', js)
    js = separators_after_closing.sub(r'\1\n\2', js)
    js = unnecessarylfs_after_statements.sub(r'\1\2 \3', js)
    # x. unnecessary semicolons & spaces
    js = js.replace(';}','}')
    js = unnecessary_spaces.sub(r'\1\2', js)
...
def js_slimmer(js, hardcore=False):
    ...
    server_side_code = re.compile(r'(\<\%.*?\%\>)', re.MULTILINE)
    lrepl=[]
    rcnt=0
    for s in server_side_code.findall(js):
        smask='@@@@@'+str(rcnt)+'@@@@'
        js=js.replace(s,smask)
        lrepl.append(s)
        rcnt=rcnt+1
    #static parser
    reg=re.compile(r'((?<!\\)\/\*|(?<!\\)\*\/|(?<!\\)\/\/|(?<!\\)[\"\'\/\*]|\n)',re.MULTILINE) # split on: ' " / /* */ // \n #"
    l=[] # [amode,string] amode: 0-code, 1-text
    ls=[]
    lf=reg.split(js)
    sm=''
    amode=0 # 0-code, 1-string/re, 2-singleline comment, 3-multiline comment
    for s in lf:
        if not s:
            continue
        if amode==0:
    ...
    ls=[]
    lastc=''
    def addCode(am,ss,lc,lr):
        if not ss:
            return lc
        if am:
            ss=_js_slimmer(ss,slim_functions=bool(hardcore))
        if not ss:
            return lc
        fc=ss[0]
        if lc in ['"',"'",')'] and ((fc>='a' and fc<='z') or (fc>='A' and fc<='Z') or (fc in ['_','$'])):
            lr.append(chr(10))
        if lc in ['/'] and ((fc>='a' and fc<='z') or (fc>='A' and fc<='Z') or (fc in ['_','$'])) and not fc in 'gmi':
            lr.append(chr(10))
        lr.append(ss)
        return ss[-1]
    for amode,s in l:
        if amode:
            lastc=addCode(1,''.join(ls),lastc,lret)
            ls=[]
            lastc=addCode(0,s,lastc,lret)
        else:
            ls.append(s)
    addCode(1,''.join(ls),lastc,lret)
    #un-protecting :) server side code
    js=''.join(lret)
    for i in range(len(lrepl)):
        smask='@@@@@'+str(i)+'@@@@'
        js=js.replace(smask,lrepl[i])
    return js
...
def run(inputfile, syntax, speedtest, outputfile, hardcore=False):
    ...
    if outputfile:
        if outputfile=='@':
            aname,aext=os.path.splitext(inputfile)
            outputfile=aname+'.min'+aext
