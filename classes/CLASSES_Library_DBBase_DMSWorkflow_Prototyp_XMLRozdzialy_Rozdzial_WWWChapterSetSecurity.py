...
def ProcessRozdzial(robj,afile,rpath):
   ...
   d={'Name':string.join(rpath,' - '),'AccessLevel':'4','mode':'update','category':'','kind':''}
   afile.TagOpen('ITEMGROUP',d,aclosetag=1,ainsertemptyattributes=1)
   afile.TagOpen('USERGROUP',d,aclosetag=1,ainsertemptyattributes=1)
...
def GetMenuXMLForProject(sfile,cobj,asecprofile):
...
def RegisterFields(aclass,amenu,file,aoid=-1,areport=None):
   ...
   if areport is None:
      ...
      cobj=aclass[aoid]
      asecprofilename=ICORSecurity.GetSecurityProfileByObj(cobj)
      asecprofile=ICORSecurity.ICORSecurityProfile()
      print 'secprofile:',asecprofilename
      if asecprofilename:
         asecprofile.SetByProfileName(asecprofilename)
      else:
         asecprofile.SetByUser(amenu.uid)
      GetMenuXMLForProject(bfile,cobj,asecprofile)
