...
def xxRegisterFields(aclass,amenu,file,aoid=-1,areport=None):
...
def xxOnBeforeWWWAction(aobj,amenu,file):
   w=1
   if 0:
...
def xxOnWWWActionGetLink(aobj,amenu):
   return ''

def xxOnWWWAction(aobj,amenu,file):
   awwweditor=RegisterFields(aobj.Class,amenu,file,aobj.OID,None)
   if amenu.Action=='ObjectApplyMethods':
      awwweditor.Write()
   return 2 # show back reference to main object (1-link, 2-button)

def xxOnWWWActionSubmit(aobj,amenu,areport,file):
