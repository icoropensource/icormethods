...
def IsImage(afname,amaxsizex=12000,amaxsizey=12000,aignoreext=0):
   ...
   try:
      ...
      if aimage.size[0]>0 and aimage.size[1]>0 and aimage.size[0]<amaxsizex and aimage.size[1]<amaxsizey:
         x=aimage.getpixel((aimage.size[0]-1,aimage.size[1]-1))
      else:
         return 0
   except:
      return 0
...
def ResizeImageByConstraint(afnamein,afnameout,max_width,max_height,amaxsizekb=-1,acrop=0,alog=None,acropproportional=0,aminquality=80):
   if alog:
      alog.Log('ResizeImageByConstraint '+afnamein+' '+afnameout+' '+str(max_width)+' '+str(max_height)+' '+str(amaxsizekb)+' '+str(acrop)+' '+str(acropproportional))
   ...
   if w:
      ...
      adata=''
      while aquality>aminquality:
         ff=cStringIO.StringIO()
         bimg=img.copy()
         bimg.save(ff,'JPEG',quality=aquality)
         adata=ff.getvalue()
         ff.close()
