...
''')
   ...
   while nobj:
      ...
      while mobj:
         ...
         #wh=0
         aurl='%s?op=%s'%(aobj.URLBase+nobj.URLPath,mobj.URLMethod)
         atext=urllib.urlopen(aurl).read()
         idiv=atext.find('<h3>SOAP 1.1</h3>')
         ibody=atext.find('</body>')
         file.write('<div class="content"><span><span>')
         file.write(XMLUtil.UTF8_To_CP1250(atext[idiv:ibody]))
         file.write('</div>')
