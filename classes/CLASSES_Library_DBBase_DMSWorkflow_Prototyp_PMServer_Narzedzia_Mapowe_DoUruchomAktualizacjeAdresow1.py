...
def ICORMain(CID=-1, FieldName='', OID=-1, Value='', UID=-1):
   ...
   sok='OK'
   smessage='Aktualizację zakończono powodzeniem'
   try:
      try:
         astate=FieldName
         fname=aICORDBEngine.Variables['_ICOR_BASE_DIR']+'/util/Korespondencja/Adresy/mi2sok1.cmd'
         os.spawnv(os.P_WAIT,fname,['"'+fname+'"',])
      except:
         sok='BAD'
         smessage='Wystąpił błąd podczas aktualizacji danych'
         import traceback
         traceback.print_exc()
         import win32api
         try:
