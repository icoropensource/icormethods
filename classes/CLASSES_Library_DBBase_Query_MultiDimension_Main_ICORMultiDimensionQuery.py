...
class FKFilter:
   def __init__(self,aoid,adimension,aparameters):
      self.ClassItem=aICORDBEngine.Classes['CLASSES_Library_DBBase_Query_MultiDimension_Filter_FKFilter']
      self.OID=aoid
      self.Dimension=adimension
      self.Name=self.ClassItem.Name[self.OID]
      self.MultiplyBy=self.ClassItem.MultiplyBy.ValuesAsFloat(self.OID)
      if self.MultiplyBy==0.0:
         self.MultiplyBy=1.0
      self.DateFrom=self.ClassItem.FromDate.ValuesAsDate(self.OID)
      if self.DateFrom==ICORUtil.ZERO_DATE_D:
         self.DateFrom=aparameters['DateFrom']
      self.DateTo=self.ClassItem.ToDate.ValuesAsDate(self.OID)
      if self.DateTo==ICORUtil.ZERO_DATE_D:
         self.DateTo=aparameters['DateTo']
...
class QueryDimension:
   def __init__(self,aoid,aparentdimension,aquery,aparameters,aonecolumnonly=0):
      self.ClassItem=aICORDBEngine.Classes['CLASSES_Library_DBBase_Query_MultiDimension_Dimension']
      self.ValuesClass=self.ClassItem.Values.ClassOfType
      self.OID=aoid
      self.ParentDimension=aparentdimension
      self.Query=aquery
      self.Name=self.ClassItem.Name[self.OID]
      self.FormulaName=self.ClassItem.FormulaName[self.OID]
      self.FormulaText=self.ClassItem.FormulaText[self.OID]
      self.OneColumnOnly=aonecolumnonly
      self.TotalName=self.ClassItem.TotalName[self.OID]
      if not self.TotalName:
         self.TotalName=self.Query.TotalColumnName+self.Name
      self.IsFramed=self.ClassItem.IsFramed.ValuesAsInt(self.OID)
      self.IsItalic=self.ClassItem.IsItalic.ValuesAsInt(self.OID)
      self.IsTotalBold=self.ClassItem.IsTotalBold.ValuesAsInt(self.OID)
      self.IsTotalBiggerFont=self.ClassItem.IsTotalBiggerFont.ValuesAsInt(self.OID)
      self.IsBold=self.ClassItem.IsBold.ValuesAsInt(self.OID)
      self.IsTotalDoubleFrameAfter=self.ClassItem.IsTotalDoubleFrameAfter.ValuesAsInt(self.OID)
      self.FrameSize=self.ClassItem.FrameSize[self.OID]
      if not self.FrameSize:
         self.FrameSize='.5pt hairline'
      self.FirstNDate=0
      self.LastNDate=0

      self.DisableTitle=self.ClassItem.DisableTitle.ValuesAsInt(self.OID)
      self.DisableTotal=self.ClassItem.DisableTotal.ValuesAsInt(self.OID)
      ...
      self.EmptyRowBefore=self.ClassItem.EmptyRowBefore.ValuesAsInt(self.OID)
      self.EmptyRowBeforeTotal=self.ClassItem.EmptyRowBeforeTotal.ValuesAsInt(self.OID)
      self.FramedTotal=self.ClassItem.FramedTotal.ValuesAsInt(self.OID) #UWAGA! Not
      ...
      self.TitleCenteredFramed=self.ClassItem.TitleCenteredFramed.ValuesAsInt(self.OID)

      if aparentdimension:
         self.ShowWn,self.ShowMa,self.ShowBOWn,self.ShowBOMa,self.ShowSaldoWn,self.ShowSaldoMa,self.ShowObrotyWn,self.ShowObrotyMa,self.ShowSaldoObrotyWn,self.ShowSaldoObrotyMa=aparentdimension.ShowWn,aparentdimension.ShowMa,aparentdimension.ShowBOWn,aparentdimension.ShowBOMa,aparentdimension.ShowSaldoWn,aparentdimension.ShowSaldoMa,aparentdimension.ShowObrotyWn,aparentdimension.ShowObrotyMa,aparentdimension.ShowSaldoObrotyWn,aparentdimension.ShowSaldoObrotyMa
      else:
      ...
      vobj=self.ClassItem[self.OID].ShowSubDimensions
      if vobj:
         self.ShowSubDimensions=ICORUtil.str2bool(vobj.Name)
      aparameters['ShowSubDimensions']=self.ShowSubDimensions
      ...
      aparameters.PushAll()
      self.FKFilters=[]
      arefs=self.ClassItem.FKFilters.GetRefList(self.OID)
      while arefs:
         afkfilter=FKFilter(arefs.OID,self,aparameters)
         ...
         self.FKFilters.append(afkfilter)
         arefs.Next()
      self.Dimensions=[]
      arefs=self.ClassItem.SubDimensions.GetRefList(self.OID)
      while arefs:
         adimension=QueryDimension(arefs.OID,self,self.Query,aparameters)
...
#         file.write('%s\n'%(' '*aindent,))
   ...
   def GetModifiedValueAsString(self,sum):
      if sum==0.0:
         ...
         if self.Query.NoDecimals:
            ssum=str(round(sum*self.DisplayValueModifier,0))
         else:
            ssum=str(round(sum*self.DisplayValueModifier,2))
      return ssum
...
class ICORMultiDimensionQuery:
   def __init__(self,aid,asriterator=None):
      ...
      self.FrameDrawing=0
      self.OneColumnOnly=self.ClassItem.OneColumnOnly.ValuesAsInt(self.OID)
      self.NoZeroValues=self.ClassItem.NoZeroValues.ValuesAsInt(self.OID)
      self.NoDecimals=self.ClassItem.NoDecimals.ValuesAsInt(self.OID)
      self.HideEmptyDimensions=self.ClassItem.HideEmptyDimensions.ValuesAsInt(self.OID)
      if self.NoDecimals:
      ...
      self.Title=self.ClassItem.Title[self.OID]
      self.SubTitle=self.ClassItem.SubTitle[self.OID]
      self.CategoryColumnName=self.ClassItem.CategoryColumnName[self.OID]
      self.TotalColumnName=self.ClassItem.TotalColumnName[self.OID]
      ...
      aparameters=ICORUtil.RecursiveParameters()
      aparameters.RegisterParameters([
   ...
   def DumpFileHTML(self,fname):
      ...
      try:
         file.write('''<html>
<head>'''+ICORWWWInterfaceUtil.GetScriptCSS()+'''
...
<table x:str border=0 cellpadding=0 cellspacing=0 width=1024 class=xl2386587 style='border-collapse:collapse;table-layout:fixed;width:768pt'>
"""%{'nazwa':self.Name,'tytul':self.Title,'podtytul':self.SubTitle,'kolumnaglowna':self.CategoryColumnName})

         file.write("<col class=xl2386587 width=477 style='mso-width-source:userset;mso-width-alt: 16952;width:358pt'><col class=xl2386587 width=20 style='mso-width-source:userset;mso-width-alt: 1393;width:14pt'>")
         for ndate in ndates:
            file.write("<col class=xl2386587 width=73 style='mso-width-source:userset; mso-width-alt:2588;width:55pt'>")
         file.write("<col class=xl2386587 width=20 style='mso-width-source:userset;mso-width-alt: 1393;width:14pt'>")
         file.write("<col class=xl2386587 width=120 style='mso-width-source:userset; mso-width-alt:3400;width:100pt'>")

         file.write("""
 <tr height=16 style='height:12.0pt'>
  <td height=16 class=xl2346587 colspan=3 width=192 style='height:12.0pt;  width:144pt'>%(tytul)s</td>
  <td class=xl2356587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2356587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2356587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2366587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2366587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2376587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2376587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2376587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2356587 width=64 style='width:48pt'>&nbsp;</td>
  <td class=xl2356587 width=64 style='width:48pt'>&nbsp;</td>
