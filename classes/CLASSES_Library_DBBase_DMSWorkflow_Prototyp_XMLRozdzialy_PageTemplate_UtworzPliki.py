...
#   awwweditor=RegisterFields(aobj.Class,amenu,file,aobj.OID,None)
   if amenu.Action=='ObjectApplyMethods':
      fclass=aobj.Class.PageFileItems.ClassOfType
      arefs=[]
      axml='  <OUTPUT filename="chapter" extension="asp">\n'
      for afname in ['PageTop1','PageLeft','PageContent','PageRight','PageBottom']:
         afield=aobj.Class.FieldsByName(afname)
         s=string.strip(afield[aobj.OID])
         if s:
            foid=fclass.AddObject(arefobject=aobj)
            axmlname=aobj.Template+' - '+afname
            fclass.Name[foid]=axmlname
            fclass.SourceData[foid]=s
            fclass.PageFileItemKind[foid]='2:1910:' #PyHTML
            arefs.append([foid,fclass.CID])
            axml=axml+'    <FILEITEM name="%s" />\n'%(axmlname)
      axml=axml+'  </OUTPUT>\n'
      s=aobj.PageXML
      s=string.replace(s,'</TEMPLATE>','')
      s=s+axml+'\n</TEMPLATE>\n'
      aobj.PageXML=s
      aobj.Class.PageFileItems[aobj.OID]=arefs
      file.write('<h1>Pliki zostały przygotowane, odśwież odpowiednie pozycje w menu</h1>')
