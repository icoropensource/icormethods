...
def OnBeforeFieldOutput(aclass,afield,aoid,asheetid,file,afields):
   if afield.Name in ['FIELD_CHART','XXUserGroups','XXItemGroups']:
      return 0

def OnAfterFieldOutput(aclass,afield,aoid,asheetid,file,afields):
   pass

def DoTreeReportUserGroups(aclass,aoid,file):
   atree=SimpleLinksHTMLTree('User Groups',atreeid='0') #enter value here
   dobj=aclass[aoid]
   aobj=dobj.UserGroups #enter value here
   while aobj:
      anode=atree.RootNode.AddNode(aobj.Name+' : <font color="green">%s</font>'%aobj.AccessLevel,'icormain.asp?jobtype=objectedit&CID=%d&OID=%d'%(aobj.CID,aobj.OID),asorted=1) #enter value here
      pobj=aobj.Users #enter value here
      while pobj:
         cnode=anode.AddNode(pobj.UserName,'icormain.asp?jobtype=objectedit&CID=%d&OID=%d'%(pobj.CID,pobj.OID),asorted=1) #enter value here
         pobj.Next()
      aobj.Next()
   atree.Write(file)

def DoTreeReportItemGroups(aclass,aoid,file):
   atree=SimpleLinksHTMLTree('Item Groups',atreeid='1') #enter value here
   dobj=aclass[aoid]
   aobj=dobj.ItemGroups #enter value here
   while aobj:
      anode=atree.RootNode.AddNode(aobj.Name+': <font color="green">%s</font>'%aobj.AccessLevel,'icormain.asp?jobtype=objectedit&CID=%d&OID=%d'%(aobj.CID,aobj.OID),asorted=1) #enter value here
      pobj=aobj.Groups #enter value here
      while pobj:
         cnode=anode.AddNode(pobj.Name,'icormain.asp?jobtype=objectedit&CID=%d&OID=%d'%(pobj.CID,pobj.OID),asorted=1) #enter value here
         pobj.Next()
      aobj.Next()
   atree.Write(file)
...
def OnAfterSheetOutput(aclass,aoid,asheetid,file,afields):
   if 'FIELD_CHART' in afields: #enter value here
      ...
      return
   if 'XXItemGroups' in afields:
      DoTreeReportItemGroups(aclass,aoid,file)
   if 'XXUserGroups' in afields:
      DoTreeReportUserGroups(aclass,aoid,file)
