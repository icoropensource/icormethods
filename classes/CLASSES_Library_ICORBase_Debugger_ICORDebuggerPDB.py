...
import os
import re
import CLASSES_Library_ICORBase_Debugger_ICORDebuggerCMD
ipdbcmd=CLASSES_Library_ICORBase_Debugger_ICORDebuggerCMD
import m32api
...
class Pdb(bdb.Bdb, ipdbcmd.Cmd):
   
   def __init__(self):
      bdb.Bdb.__init__(self)
      ipdbcmd.Cmd.__init__(self)
      self.prompt = '(Pdb) '
      self.aliases = {}
...
#      self.print_stack_entry(self.stack[self.curindex])
   ...
   def default(self, line):
      ...
      m32api.f_fast_to_locals(self.curframe)
...
#         m32api.f_fast_to_locals(self.curframe)
         for k,v in locals.items():
            m32api.f_set_in_locals(self.curframe,k,v)
      except:
         t, v = sys.exc_info()[:2]
         if type(t) == type(''):
            exc_type_name = t
      ...
      m32api.f_locals_to_fast(self.curframe)
...
#      clear bpno bpno ... -> clear breakpoints by number"""
      if not arg:
         try:
            ...
            reply = 'no'
         reply = string.strip(reply).lower()
         if reply in ('y', 'yes'):
