...
def DumpClassXML(file,aclass,adate):
   ...
   if not aclass.IsSystem=='1' and not aclass.IsVirtual=='1':
      ...
      if adt<adate:
         return
      amethod=aclass.MethodsByName('OnClassExportModifications')
      ...
      aclasswritten=0
      dlist=aclass.GetDeletedObjectsList(adate)
      if len(dlist)>0:
         file.write(sclass)
         aclasswritten=1
         file.write('  <DELETED>\n')
      ...
      for afname in flist:
         ...
         if afield.IsInteractive=='1' and afield.AllowRead=='1' and afield.IsVirtual!='1' and afield.IsReportProtected!=1:
            ...
            sd=tdatetime2str(adt,' ')
            sdf='  <FIELD Name="%s" LastModification="%s">\n' % (afname,sd)
            fieldwritten=0
...
def GenerateAcceptation(afname,adatefrom=(),adatecreated=(),atargetuser=0,arootclass=None):
   import gzip
   if adatefrom==():
...
def ICORMain(CID=-1, FieldName='', OID=-1, Value='', UID=-1):
   ...
   GenerateAcceptation('c:\\tmp\\aaa.gz',(1998,10,1,0,0,0,0),(),-1,aclass)
   return
   ...
   if not adialog.Show():
      return
   aclass=aICORDBEngine.Classes[adialog.ClassPath]
   GenerateAcceptation(afile,(1998,10,1,0,0,0,0),(),-1,aclass)
