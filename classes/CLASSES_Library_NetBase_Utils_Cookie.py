...
#jrr        if string.lower(key) in self.__reserved_keys:
#jrr            raise CookieError("Attempt to set a reserved key: %s" % key)
        self.key                 = key
        self.value               = val
        self.coded_value = coded_val
    # end set

    def output(self, header = "Cookie:"):
...
class Cookie(UserDict):
    ...
    def load(self, rawdata):
        ...
        dictionaries!
        """
        if isinstance(rawdata,types.StringTypes):
